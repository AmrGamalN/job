...
class SaveController {
  ...
  getSave = async (req: Request, res: Response) => {
    const result = await this.saveService.getSave(
      req.params.id,
      req.curUser.userId
    );
    return controllerResponse(res, result);
  ...
  getAllSaves = async (req: Request, res: Response) => {
    const result = await this.saveService.getAllSaves(req.query, req.curUser);
    return controllerResponse(res, result);
  };

  countSave = async (req: Request, res: Response) => {
    const result = await this.saveService.countSave(req.query, req.curUser);
    return controllerResponse(res, result);
  };
